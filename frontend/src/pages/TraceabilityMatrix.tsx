...
import type { Requirement, TraceabilityLink } from "../types";
...
export function TraceabilityMatrix() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([requirementsApi.list(currentProject.id), traceabilityApi.matrix(currentProject.id)])
      .then(([reqs, matrixLinks]) => {
  ...
  const coveredCount = requirements.filter((r) => testSectionsByRequirement.has(r.id)).length;
  ...
  return (
    <div>
      ...
      <div className="stat-grid">
        ...
        <div className="stat-card">
          <div className="stat-value">{requirements.length - coveredCount}</div>
